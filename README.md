...
**A "Wolta measured a different battery capacity" repair appears (v0.12.0+).** Once there are a couple of months of history, Wolta measures your battery's usable capacity at the meter and, if it clearly disagrees with what you configured, raises a fixable repair with the measured value. Confirming it sets the battery capacity to the measured figure and **clears any reserve floor** — the measurement already reflects only the window you actually cycle, so the reserve must not be subtracted again. It only appears when the reconstruction is mature and confident (≥60 days, a repeated ceiling, and a gap over ~15 %); if you simply haven't fully charged or discharged in the period, ignore and dismiss it. The measured value is the AC/meter-side usable capacity, not the DC/cell figure your battery app shows.
