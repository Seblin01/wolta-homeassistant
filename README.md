...
Either way, an interval is only ever uploaded when Home Assistant has compiled energy statistics for that exact 15 minutes. A held-forward flag can therefore never invent intervals across a stretch where the recorder has nothing — while Home Assistant was down, for instance — so a stale state cannot turn a gap into a wall of empty "externally controlled" intervals. From the first upload onwards, flagging keeps pace with every new upload as long as the sensor stays configured.
